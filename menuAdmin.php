...
class MenuAdmin extends Menu{
      ...
      public function __construct($atletaManager, $carreraManager){
        $this->atletaManager = $atletaManager;
        $this->carreraManager = $carreraManager;
      }
       
      //Pantalla menú principal para un administrador
      private function mostrarOpcionesAdmin(){
	self::cls();          
    ...
    //Pantalla ABM carreras para un administrador
    private function mostrarOpcionesABMCarreras(){
    ...
    private function bajaCarrera(){
        $id = self::readln("Ingrese Id de carrera a eliminar: ");
        if($this->carreraManager->existeId($id)){
        		$this->carreraManager->eliminarPorId($id);
        	}else {
        		self::writeln("El id ingresado no se encuentra entre nuestras carreras");
        }
  }

      //Un administrador va a operar con carreras
      protected function ABMcarreras(){
        $opcion = self::mostrarOpcionesABMcarreras();     //0 volver, 1 alta, 2 baja, 3 modificacion, 4 mostrar
        while ($opcion != 0){
            switch ($opcion) {
                case '1': 
                        self::altaCarrera();
                ...
                case '3':
                        ...
                        break;
                case '4':
                        $this->carreraManager->mostrarCarreras();
                        break;
                default:
                       self::writeln("Tipo de operación inválida");
                       break;
                }
                $opcion = self::mostrarOperacionesABMCarreras();     //0 volver, 1 alta, 2 baja, 3 modificacion, 4 mostrar
        }
    }
    ...
    //Se le presentan todas las opciones para operar a un Administrador
        public function operacionesAdmin(){
                $opcion = self::mostrarOpcionesAdmin();     //0 volver, 1 carreras, 2 participantes, 3 pagos
                while ($opcion != 0){
                switch ($opcion) {
                        case '1': 
                                self::ABMCarreras();
                ...
                $self::admin();  //0 salir, 1 participante, 2 administrador
                $opcion = self::readline("opcion: ");
        }
    }
...
while ($opcionTipoUsuario != 0){
     ...
     switch ($opcionTipoUsuario) {
        case '1': 
	   $menuParticipante = new MenuParticipante($atletaManager);
           break;
        case '2':
